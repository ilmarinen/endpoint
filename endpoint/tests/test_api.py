...
class TestCase(unittest.TestCase):
    ...
    def test_create_restaurant_owner(self):
        user_a = api.create_user("max", "Max", "Planck", "max@test.com", restaurant_owner=True, is_active=True)
        user_b = api.create_user("heisenberg", "Werner", "Heisenberg", "h@test.com", restaurant_owner=False, is_active=True)

        assert(api.is_restaurant_owner(user_a))
        assert(not api.is_restaurant_owner(user_b))
